...
matrices.append(transform_matrix_from_rotation_matrix(quaternion.as_rotation_matrix(q0)))
...
for i, matrix in enumerate(matrices[:nframes]):
    ...
    #pltr.set_viewup(matrix[:, 1])
    pltr.camera.zoom(0.9)
#    for light in pltr.renderer.lights:
#        light.set_direction_angle(pltr.camera.elevation, pltr.camera.azimuth)
#        light.transform_matrix = matrix
#        light.position = matrix[0:3, 0:3].dot(light.position)
...
#    pltr.renderer.ResetCameraClippingRange()
    for circle in circles:
        ...
        pltr.add_mesh(sphere, texture=tex)# smooth_shading=True, color="red", specular=10)
    #     #
